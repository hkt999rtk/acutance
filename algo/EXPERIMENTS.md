...
1. full parity analysis: `gamma=0.5 + demosaic_red`
- 40 份整批結果：
  - `curve_mae_mean = 0.13126`
  - `Phone = 0.02781`
  - `Monitor = 0.17853`
  - `UHDTV = 0.13941`
  - `Small Print = 0.10182`
  - `Large Print = 0.12281`
- 結論：
  - 誤差非常大，不適合直接當新的 recommended profile
  - 問題不是小幅參數差，而是整條 MTF/PSD 曲線系統性失配

2. parity residual analysis: `gamma=0.5 + demosaic_red`
- 40 份整批 `MTF` signed relative error：
  - `low = -10.20%`
  - `mid = -38.47%`
  - `high = -71.33%`
  - `top = -75.99%`
- 結論：
  - 不是單純 frequency scale 問題
  - parity gamma 0.5 會把中高頻 MTF 壓得太低

3. parity frequency scale sweep: `gamma=0.5 + demosaic_red`
- 最佳 `frequency_scale = 1.175`
- 結論：
  - 比舊基線略高
  - 但不足以解釋整體失配，不是主因

4. sample linearization sweep under `demosaic_red`
- sample:
  - [OV13b10_AG1_ET40000_deadleaf_12M_1_denoised.raw](/Users/kevinhuang/work/acutance/20260318_deadleaf_13b10/output2_Bmodel/OV13B10_AI_NR_OV13B10_20251205_v2_2650_ppqpkl_0.25/OV13b10_AG1_ET40000_deadleaf_12M_1_denoised.raw)
- 最佳線性化候選反而是：
  - `gamma = 1.0`
  - `white_percentile = 99.5`
- 結論：
  - `Gamma, 0.5` 更像是報告 metadata / OECF 設定
  - 而不是 dead-leaves MTF 分析真正使用的 gamma

5. split profile: `report gamma = 0.5`, `analysis gamma = 1.0`, `bayer_mode = demosaic_red`
- 40 份整批結果：
  - `curve_mae_mean = 0.04691`
  - `Phone = 0.00677`
  - `Monitor = 0.06849`
  - `UHDTV = 0.03917`
  - `Small Print = 0.03063`
