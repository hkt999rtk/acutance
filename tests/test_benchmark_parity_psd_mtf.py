...
class BenchmarkParityPsdMtfTest(unittest.TestCase):
    ...
    def test_intrinsic_scope_can_disconnect_reported_mtf_from_intrinsic_reconnect(self) -> None:
        capture_key = "OV13b10_AG8_ET5500_deadleaf_12M_40"
        with tempfile.TemporaryDirectory() as tmpdir:
            dataset_root = Path(tmpdir)
            variant_root = dataset_root / "OV13B10_AI_NR_OV13B10_ppqpkl_0.10"
            ...
            self.assertEqual(apply_correction_curve.call_count, 0)
            np.testing.assert_allclose(compute_metrics.call_args.args[0], [0.02, 0.1, 0.25, 0.4])
            np.testing.assert_allclose(compute_metrics.call_args.args[1], [1.0, 1.0, 1.0, 1.0])
    ...
    def test_intrinsic_scope_can_graft_pr30_bundle_to_readout_without_main_acutance(self) -> None:
        capture_key = "OV13b10_AG8_ET5500_deadleaf_12M_40"
        with tempfile.TemporaryDirectory() as tmpdir:
            dataset_root = Path(tmpdir)
            variant_root = dataset_root / "OV13B10_AI_NR_OV13B10_ppqpkl_0.10"
            ...
            def apply_compensation(mtf, freqs, *, mode, **_kwargs):
                values = np.asarray(mtf, dtype=np.float64)
            ...
            with (
                ...
                patch("algo.benchmark_parity_psd_mtf.parse_imatest_random_csv", side_effect=parse_csv),
                patch(
                    "algo.benchmark_parity_psd_mtf.load_raw_u16",
                    side_effect=lambda path, width, height: np.zeros((height, width), dtype=np.uint16),
                ),
                ...
                patch("algo.benchmark_parity_psd_mtf.estimate_dead_leaves_mtf", return_value=estimate),
                patch(
                    "algo.benchmark_parity_psd_mtf.estimate_texture_support_scale",
                    return_value=SimpleNamespace(frequency_scale=1.0),
                ),
                patch("algo.benchmark_parity_psd_mtf.apply_mtf_compensation", side_effect=apply_compensation),
                patch(
                    "algo.benchmark_parity_psd_mtf.derive_intrinsic_transfer_curve",
                    return_value=np.array([2.0, 2.0, 2.0, 2.0], dtype=np.float64),
                ),
            ...
            np.testing.assert_allclose(compute_metrics.call_args.args[1], [6.0, 6.0, 6.0, 6.0])
            np.testing.assert_allclose(acutance_from_mtf.call_args.args[1], [2.0, 2.0, 2.0, 2.0])
            self.assertEqual(
                payload["analysis_pipeline"]["calibration_file"],
                "algo/deadleaf_13b10_psd_calibration_anchored.json",
            )
            self.assertEqual(payload["analysis_pipeline"]["mtf_compensation_mode"], "sensor_aperture_sinc")
            self.assertEqual(payload["analysis_pipeline"]["sensor_fill_factor"], 1.5)
            self.assertTrue(payload["analysis_pipeline"]["texture_support_scale"])
            self.assertEqual(payload["analysis_pipeline"]["high_frequency_guard_start_cpp"], 0.36)
