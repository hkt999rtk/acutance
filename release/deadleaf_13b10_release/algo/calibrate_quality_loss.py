...
def summarize_accumulator(accumulator: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "preset_mae": preset_mae,
        "overall_mae_mean": float(np.mean(list(preset_mae.values()))) if preset_mae else None,
    }


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Benchmark Quality Loss against Imatest CSV")
...
def main() -> int:
    ...
    for csv_path in csv_paths:
        ...
        quality_loss = quality_loss_presets_from_acutance(acutance)
        preset_cmp = {
            name: abs(value - reference.reported_quality_loss[name])
            for name, value in quality_loss.items()
        }

        labels = classify_csv(csv_path, args.dataset_root)
        append_metrics(overall, preset_cmp=preset_cmp)
        append_metrics(
