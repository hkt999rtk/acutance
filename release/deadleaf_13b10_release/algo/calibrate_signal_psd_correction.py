...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Sweep signal PSD mid-band correction gain")
    ...
    parser.add_argument("--gain-values", type=float, nargs="+", default=[0.0, 0.05, 0.1, 0.15, 0.2, 0.25])
    parser.add_argument("--start-cpp", type=float, default=0.08)
    parser.add_argument("--peak-cpp", type=float, default=0.15)
    parser.add_argument("--stop-cpp", type=float, default=0.22)
    ...
    for gain in args.gain_values:
        ...
        for image, roi, reference in samples:
            estimate = estimate_dead_leaves_mtf(
                ...
                signal_psd_correction_gain=gain,
                signal_psd_correction_start_cpp=args.start_cpp,
                signal_psd_correction_peak_cpp=args.peak_cpp,
                signal_psd_correction_stop_cpp=args.stop_cpp,
